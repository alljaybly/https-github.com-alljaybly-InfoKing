...
const MockupModal: React.FC<MockupModalProps> = ({ idea, mockupImage, isLoading, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-md flex justify-center items-center z-50 animate-fade-in p-4" onClick={onClose}>
      <div className="bg-white/50 dark:bg-gray-800/50 backdrop-blur-lg rounded-xl shadow-2xl w-full max-w-lg m-auto p-6 md:p-8 animate-slide-in" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="w-full aspect-[9/16] bg-gray-200 dark:bg-gray-700 rounded-lg flex items-center justify-center overflow-hidden border border-white/10">
            {isLoading && (
            ...
            {mockupImage && !isLoading && (
                <img src={mockupImage} alt="AI-generated app mockup" className="w-full h-full object-contain" />
            )}
            {!mockupImage && !isLoading && (
